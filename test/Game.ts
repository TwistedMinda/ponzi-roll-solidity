...
describe("Game", function () {
	...
	describe('Workflow', () => {
		...
		it("Should upgrade round", async () => {
			...
			await playForLoss(owner, game, randomizer, VRFCoordinatorV2Mock)
	...
	describe('Player', () => {
		...
		it("Stats increasing", async () => {
			const { otherAccount, game, randomizer, VRFCoordinatorV2Mock } = await loadFixture(deploy);
			...
			await playForWin(target, game, randomizer, VRFCoordinatorV2Mock)
			await playForLoss(target, game, randomizer, VRFCoordinatorV2Mock)
			await time.increase(ROUND_DURATION)
			await playForLoss(target, game, randomizer, VRFCoordinatorV2Mock)
